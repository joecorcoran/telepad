...
Telepad.prototype.playSound = function(sound, idx) {
  ...
  if (callSid) {
    ...
    this.readyRow(idx);
    this.launchpad.getButton(sound, idx).light(this.launchpad.colors.green.high);
    this.twilio.calls(callSid).update({
...
Telepad.prototype.readyRow = function(idx) {
  for(var i = 0; i < 8; i++) {
    this.launchpad.getButton(i, idx).light(this.launchpad.colors.yellow.low);
  }
};
